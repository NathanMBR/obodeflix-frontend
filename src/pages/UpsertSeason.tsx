...
export const UpsertSeason = () => {
  ...
  const handleUpsertSeasonSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    const upsertSeasonPayload = {
      ...
      tracks: tracks.map(({ id: _id, ...trackWithoutId}) => trackWithoutId)
    };
  ...
  const handleTracksChange = (tracks: Array<Omit<Track, NewTrackFieldsToOmit>>) => {
    setTracks(tracks);
  }
